...
Xchat `0.1.4` is a LAN chat client built with Tauri 2, React, and Rust. Install and run the client on each device; discovery, messaging, file transfer, and SQLite storage are built in, so normal desktop use does not require a separate server.
...
src-tauri/target/release/bundle/dmg/Xchat_0.1.4_*.dmg
```
...
The internal Rust package and compatibility binaries remain named `lanchat` / `lanchat-web`. The visible app name, version, and bundle identifier are `Xchat`, `0.1.4`, and `com.xchat.app`.
