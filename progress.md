...
- 执行的操作：
  ...
  - 功能分支已通过非交互式 merge 合入本地 `main`，合并提交为 `72c6a3271cf09d3f9551df141f42db2d96ce4beb`；合并结果再次通过前端 101/101 与 Rust 144/144。
- 后续真机验收：
  - 两个网段的设备都安装本次新版本时可验证协商后的 8/16；任一端为旧版本时会按设计固定使用 4 通道。
